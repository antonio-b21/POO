...
istream& operator >> (istream& os, Sistem& sis) {
	for (int i = 0; i < sis.nrEcuatii; i++) {
		cout << "Ecuatia " << i + 1 << ":\n";
		cout << "F1*x + F2*y = F3\n";
		sis.ec[i] = CitMemAfis(sis.nrEcuatii + 1); //da eroare cand salvez headerul si metodele in fisiere separate
		cout << "-----------------------------\n";
	}
	sis.DeterminaRang();
	return os;
...
Fractie Sistem::CalculeazaDet(int col) {
	...
	Fractie det1, det2, det;
	det1 = ec[0][in[0]] * ec[1][in[1]];
	det2 = ec[1][in[0]] * ec[0][in[1]];
	det = det1 - det2;
	delete[] in;
	return det;
}
